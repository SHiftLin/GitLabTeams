...
parser.add_argument('labpath', metavar='labpath', type=str, nargs='?',
                    default='/root/dslabs/dslabs-master', help='path to the lab directory')
args = parser.parse_args()

args.labname = labnames[args.labnum[0]]
...
for repo in repos:
    ...
    src = "%s/labs/%s/src" % (repopath, args.labname)
    dst = "%s/labs/%s/src" % (args.labpath, args.labname)
